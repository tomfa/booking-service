...
const createThirdPartyToken = async (thirdPartyData: JSONObject) => {
...
describe('verify', () => {
  ...
  it.skip('accepts tokens issued by accepted third parties', async () => {
    cache.clear();
    const validIssuer = 'TODO: issuer from database';
    const thirdPartyData = {
      ...data,
      iss: validIssuer,
      exp: Math.floor(Date.now() / 1000) + 2000,
    };
    const { token, keystore } = await createThirdPartyToken(thirdPartyData);
  ...
  it.skip('utilizes a cache to prevent frequent retrieval of third party JWK keys', async () => {
    cache.clear();
    const validIssuer = 'TODO: issuer from database';
    const thirdPartyData = {
      ...data,
  ...
  it.skip('rejects tokens issued by accepted third parties, if no jwks endpoint exists', async () => {
    cache.clear();
    const validIssuer = 'TODO: issuer from database';
    const thirdPartyData = {
      ...data,
    ...
    const { token } = await createThirdPartyToken(thirdPartyData);
    nock(`https://${validIssuer}`).get('/.well-known/jwks.json').reply(404);
  ...
  it.skip('rejects tokens issued by unknown third parties', async () => {
    const unknownIssuer = 'iamnotreal.com'; // TODO: Needs to mock database
    const thirdPartyData = {
      ...data,
      iss: unknownIssuer,
      exp: Math.floor(Date.now() / 1000) + 2000,
...
describe('Keystore/keys management', () => {
  ...
  const keys = getPublicPrivateKeyPair();
  beforeAll(async () => {
